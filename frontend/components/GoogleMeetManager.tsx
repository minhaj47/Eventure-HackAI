...
export const GoogleMeetManager: React.FC<GoogleMeetManagerProps> = ({
  ...
  const [createdMeeting, setCreatedMeeting] =
    useState<GoogleMeetResponse | null>(null);
  ...
  const [meetingData, setMeetingData] = useState<GoogleMeetData>({
    ...
    endDateTime: eventData.datetime
      ? new Date(new Date(eventData.datetime).getTime() + 60 * 60 * 1000)
          .toISOString()
          .slice(0, 16)
      : "",
    editorEmail: "",
  ...
  const handleCreateMeeting = async () => {
    ...
    try {
      ...
      if (!isAuthenticated) {
        throw new Error(
          "Authentication required. Please sign in with your Google account to create Google Meet meetings."
        );
      }
      ...
      if (!meetingResponse.meetingUrl && !meetingResponse.meetingTitle) {
        ...
        const fallbackResponse = {
          ...
          instructions:
            "Google Meet created successfully. Please check your Google Calendar for the meeting link.",
        };
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to create Google Meet. Please try again."
      );
    } finally {
      setIsCreating(false);
    }
    ...
    <div className="space-y-6">
            ...
            {existingMeetings.map((meeting, index) => (
              ...
              >
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <Button
                    onClick={() =>
                      copyToClipboard(meeting.meetingUrl, "Meeting URL")
                    }
                    label="Copy Link"
                  ...
                  {meeting.calendarLink && (
                    <Button
                      onClick={() =>
                        window.open(meeting.calendarLink, "_blank")
                      }
                      label="View Calendar"
      ...
      {(showCreateForm ||
        ...
        <>
          {status === "loading" ? (
            /* Loading Session */
            <Card title="Loading..." icon={<Video className="h-5 w-5" />}>
              ...
              <div className="text-center space-y-4">
                <div className="p-4 bg-yellow-900/20 border border-yellow-500/30 rounded-lg">
                  ...
                  <p className="text-gray-300 text-sm">
                    Please sign in with your Google account to create and manage
                    Google Meet meetings for your events.
                  </p>
                </div>
                ...
                <p className="text-xs text-gray-500">
                  Sign in using the authentication button in the app header to
                  access Google Meet functionality.
                </p>
              </div>
            </Card>
          ) : !createdMeeting ? (
                  ...
                  <div className="space-y-4">
                    <div>
                      <Input
                        ...
                        onChange={(value) =>
                          handleInputChange("meetingTitle", value)
                        }
                    ...
                    <div>
                      <Input
                        ...
                        onChange={(value) =>
                          handleInputChange("editorEmail", value)
                        }
                        placeholder="admin@example.com (optional)"
                      />
                      <p className="text-xs text-gray-400 mt-1">
                        Email address for meeting admin access (uses your
                        account if empty)
                      </p>
                    </div>
                  </div>
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-green-900/20 border border-green-500/30 rounded-lg">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-green-300 text-sm">
                        Your Google Meet is ready and has been added to your
                        calendar
                      </p>
                    </div>
                  </div>
                  ...
                  {/* Co-host Information */}
                  {(createdMeeting.editorEmail ||
                    createdMeeting.displayData?.coHost) && (
                  ...
                  <div className="flex flex-wrap gap-3">
                      ...
                      onClick={() =>
                        window.open(
                          createdMeeting.meetingUrl ||
                            createdMeeting.displayData?.meetLink,
                          "_blank"
                        )
                      }
                      label="Join Meeting"
                    ...
                    {(createdMeeting.calendarLink ||
                      createdMeeting.displayData?.calendarLink) && (
                      <Button
                        onClick={() =>
                          window.open(
                            createdMeeting.calendarLink ||
                ...
                <div className="p-3 bg-gray-800/30 border border-gray-600/30 rounded-lg">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-gray-400 text-sm">
                      Meeting scheduled and ready to join
                    </p>
      ...
      >
        <div className="space-y-4 text-sm text-gray-300">
          ...
          <div>
            ...
            <ul className="space-y-1 list-disc list-inside text-gray-400">
              <li>
                Meeting times are automatically set based on your event schedule
              </li>
              <li>
                Leave editor email empty to use your authenticated account
              </li>
              <li>The meeting URL can be shared directly with participants</li>
              <li>
                Meeting recordings and settings can be managed in Google Meet
              </li>
            </ul>
          </div>
        </div>
